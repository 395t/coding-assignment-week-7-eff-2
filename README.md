...
Note that Linformer model is only supported by a specific branch of Hugging Face's transformers library.



The version can be installed by the following commands:
...
All training is performed on a Linux machine with GTX3070 GPU.
...
As shown in the tables, big bird model obtains a slight better result (i.e., 91.03% v.s. 90.79%) in terms of accuracy. On the other hand, Linformer model is two times faster than the Big bird model. We believe it is because of the reduced complexity of Linformer model.
...
Finally, we demonstrate the loss curve of the Linformer model and the Big bird model. According to the figure, the Big bird model converged faster than the Linformer model. Thus, it also shows more slightly better accuracy in the QA tasks.
